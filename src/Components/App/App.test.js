...
describe('App', () => {
  ...
  it('Should be able to view listings for an area', async() => {
    const { getByText, getAllByText, getByPlaceholderText } = render(<MemoryRouter>
      <App  />
      </ MemoryRouter>)
    fireEvent.change(getByPlaceholderText("name"), {
      target: { value: "Olivia" },
    ...
    fireEvent.click(getByText("Log In"));
    const viewListingsButton = await waitFor(() => getAllByText('View Listings'))
    fireEvent.click(viewListingsButton[1])
    const areaListings = await waitFor(() => getByText('Updated Park Hill Duplex'))
  ...
  it('Should be able to view details for a listing', async() => {
    ...
    const listingDetail = await waitFor(() => getByText('This rental is $165 per night.'))
    expect(listingDetail).toBeInTheDocument();
  })

  it('Should be able to view favorites', async() => {
    const { getByText, getAllByText, getByAltText, getByPlaceholderText } = render(<MemoryRouter>
      <App  />
      </ MemoryRouter>)
    fireEvent.change(getByPlaceholderText("name"), {
      target: { value: "Olivia" },
    ...
    const viewFavoritesButton = await waitFor(() => getByText('View Favorites: 1'))
    fireEvent.click(viewFavoritesButton)
    const favoriteListing = await waitFor(() => getByText('Updated Park Hill Duplex'))
  ...
  it('Should show a message if there are no favorites', async() => {
    ...
    const noFavoriteMessage = await waitFor(() => getByText('You currently do not have any listings saved to favorites.'))
